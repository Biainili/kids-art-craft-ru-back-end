  ...
  if (text === "/start") {
    ...
    await bot.sendSticker(
      ...
      "https://tlgrm.eu/_/stickers/5ff/185/5ff185b6-6457-4128-b22e-c18477ad084e/1.webp",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text:`${userLanguage ==='ru' ? `Открыть Приложение` : `Open App`}`, web_app: { url: webAppUrl } }],
          ],
        },
      }
    );
...
bot.on("callback_query", (callbackQuery) => {
  ...
  if (callbackData.startsWith("approve_") && order) {
    ...
    bot.sendMessage(
      ...
      🗣 ${language === "am" ? `Ստացեք 10% զեղչ այս պրոմոկոդով` : `Получите 10% скидку по этому промокоду`} <b>--- <code>KIDS2024</code></b>`,
      { parse_mode: "HTML" }
    );
